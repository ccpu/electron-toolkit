...
1. Copy this entire `templates/app/new-window` directory to `app/windows/[your-widget]`
2. Update the window name in the files (replace `widget-` placeholders)
...
find app/windows/about -type f -name "*.json" -o -name "*.tsx" -o -name "*.ts" | xargs sed -i 's/widget-/about/g'
